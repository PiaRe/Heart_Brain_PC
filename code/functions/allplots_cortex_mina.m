function allplots_cortex_mina(sa, data, colorlimits, cm, unit, smooth, varargin)
%% varargin options
...
%% Notes
% The function is taken from Stefan Haufe. It has been modified by Mina Jamshidi. 
...
surface_pars = struct('alpha_const', 1, 'mycolormap', cm, 'colorlimits', colorlimits, ...
...
if ismember(1,views)    
    figure; 
    showsurface3(vc, sa.cortex75K.tri_left, surface_pars, data);
    if DoSave
...
if ismember(2,views)
    figure; 
    showsurface3(vc, sa.cortex75K.tri_right, surface_pars, data);
    if DoSave
...
if ismember(5,views)    
    figure; 
    showsurface3(vc, sa.cortex75K.tri, surface_pars, data);
    if DoSave
...
if ismember(6,views)
    ...
    figure; 
    showsurface3(vc, sa.cortex75K.tri, surface_pars, data);%upperview rotated
    if DoSave
