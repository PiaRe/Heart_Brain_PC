function [EEG, rejectDataIntervals] = trimOutlier(EEG, amplitudeThreshold_hf, pointSpreadWidth)
    ...
    absMinMaxAllChan = max([abs(min(EEG_mfront.data(:, 1001:end))); abs(max(EEG_mfront.data(:, 1001:end)))], [], 1);
    ...
    absMinMaxAllChan_hf = max([abs(min(EEG_hf.data(:, 1001:end))); abs(max(EEG_hf.data(:, 1001:end)))], [], 1);
    ...
    if any(badPoints)
        ...
        rejectDataIntervals = rejectDataIntervals + 1001; %to account for datapoints taken away at the beginning.
        ...
        for kk = 2:size(rejectDataIntervals, 1) %skip 1 because it's the first 2 sec
